...
import type { ApprovedReview } from "@/types/api";
...
export function ReviewsSection() {
  ...
  useEffect(() => {
    ...
    async function loadReviews() {
      try {
        ...
        const data: ApprovedReview[] = await response.json();
        if (active) {
          setReviews(data);
        }
      } finally {
  ...
  const submitReview = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus({
        type: "error",
        text: "No pudimos guardar la reseña. Intenta de nuevo."
      });
    } finally {
              ...
              <div className="mt-5">
                ...
                >
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      {star <= (hoveredStar || rating) ? <FaStar size={27} /> : <FaRegStar size={27} />}
                    </button>
                  ))}
                </div>
              </div>
