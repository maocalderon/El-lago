...
import { sendReservationNotification } from "@/lib/reservationEmail";
import type { ZoneSlug } from "@/types/api";
...
const ZONE_TIMES: Record<string, { min: string; max: string }> = {
  restaurante: { min: "09:00", max: "18:00" },
  piscina: { min: "09:00", max: "18:00" },
  "salon-eventos": { min: "00:00", max: "23:59" },
};
...
function validateReservation(values: ReservationFormState) {
  ...
  if (!values.zone) {
    errors.zone = "Selecciona una zona.";
  }
  ...
  if (values.zone === "salon-eventos" && !values.end_time) {
    errors.end_time = "Selecciona la hora fin.";
  }
...
export function ReservationForm() {
  ...
  const isSalon = form.zone === "salon-eventos";
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus({
        type: "error",
        text: `No pudimos guardar la reserva o enviar el correo. ${message}`
      });
    } finally {
  ...
  const zoneConstraints = form.zone ? ZONE_TIMES[form.zone] : null;
      ...
      <div className="section-shell grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        <Reveal>
          <SectionHeading
            ...
            description="Elige la zona y completa tus datos. Te contactaremos para confirmar la disponibilidad."
          />
        ...
        <Reveal delay={0.08}>
          ...
          >
              ...
              <FormField
                ...
                input={
                  <input
                    ...
                    type="time"
                    min={zoneConstraints?.min}
                    max={zoneConstraints?.max}
                  />
                }
              />
              {isSalon && (
            ...
            <button
              ...
              disabled={isSubmitting || !form.zone}
            >
              <FiSend size={18} />
...
function FormField({
  ...
  return (
    <label className="mt-4 block text-sm font-semibold text-lago-navy dark:text-white">
      {label}
